...
using namespace std;
	...
	ofstream zapis("zapisz.txt");
		...
		macierz::macierz() // konstruktor bez podanego parametru
		{
		...
		macierz::macierz(int n) //konstruktor z podanym parametrem
		{
			rozmiar = n;
		...
		{
			int k = (wierzcholki*(wierzcholki - 1)*procent) / 200; //przeksztalcony wzor na gestosc
			return k;
		}
		...
		void macierz::wypelnijLosowoMacierz(int wierzcholki, int procent)
		{
			...
			int i = 0, m, n, l = wierzcholki - 2,  tab[500], w; //tab - tablica pomocnicza
			
			for (int i = 0; i < wierzcholki; i++) //laczy pierwsze 10 krawedzi
			{
				
				m = i;//poczatek krawedzi
				if (i == (wierzcholki - 1))
					m = 0; 
				else
					n = i + 1; // Wierzchołek docelowy krawędzi

				w = rand() % 10 + 1;                    // Waga krawędzi od 1 do 10
				this->tab[m][n] = w;
			...
			for (int j = l; j > 0; j--)  //laczy pozostale krawedzie (przekatne)
			{
				tab[i] = j;
				i++;
			}
		...
		graf::graf(int n)  //konstruktor parametryczny przyjmujacy liczbe wierzcholkow
		{
			wierzcholek = n;
			dojscia = new int[n];
			...
			Stos = new int[n];                // Stos potrzebny do wypisywania sciezek
			wsk = 0;
		}

		void graf::poczatek(int n, int v)//liczba wierzcholkow i wierzcholek poczatkowy
		{
			for (int i = 0; i < n; i++)
			{
				dojscia[i] = MAXINT;
		...
		void graf::wypelnijLosowo(int wierzcholki, int procent) //funkcja tworzaca krawedzie w grafie dla zadanej gestosci
		{
			//int k = wyliczKrawedzie(wierzcholki, procent);
		...
		void graf::wyznaczSciezkiMacierz(int n)  //korzystamy z algorytmu Dijkstry
		{
		...
		void graf::wyswietlWynikNajDrogi(int n,int w) //znajduje najkrotsza sciezke miedzy danymi wierzcholkami
		{
			int i, j;
			for (i = 0; i < n; i++)
			{
		...
		void graf::zapiszWyniki(int n) //zapisje wyniki do pliku
		{
			int i, j;
			for (i = 0; i < n; i++)
			{
		...
		void graf::zapiszWynikiSciezki(int n, int w) //zapisuje sciezki do pliku
		{
			int i, j;
			for (i = 0; i < n; i++)
			{
